...
export async function run(options: TailwintOptions = {}): Promise<number> {
  ...
  try {
    ...
    // Skip CSS files with no Tailwind signals.
    ...
    // Wait for initialization and diagnostics for every opened file.
    ...
    spinners.push(stopScan);
    await waitForDiagnostics([...fileContents.keys()].map(fileUri), timeoutMs);
    spinners.pop()!();
